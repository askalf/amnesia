...
# copy infra/ contents here: docker-compose.yml, docker-compose.vpn.yml,
# searxng/, searxng-proxy/, .env.example
cp .env.example .env
# fill .env:
...
The live shape needs no proxy: `searxng/settings.yml` sets none and gluetun's
HTTP proxy is off, so engine connections go straight into the tunnel. The
proxy shape mounts `searxng-proxy/` instead, the same settings plus the proxy
line (generated by `scripts/searxng-proxy-settings.mjs`; SearXNG ignores
`HTTP_PROXY` from the environment, so that line is what keeps it on the VPN).
...
`enable_metrics` is on, so SearXNG keeps per-engine response times, timeouts
and errors since the container started. Only the host can see them: the Worker
forwards `/search` and `/autocompleter` and nothing else.

```sh
# From your machine: forward the loopback port, then open the two pages.
ssh -i ~/.ssh/askalf_platform_ed25519 -L 8081:127.0.0.1:8081 root@178.104.181.103
#   http://127.0.0.1:8081/stats          response time, reliability per engine
#   http://127.0.0.1:8081/stats/errors   timeouts and errors per engine
```

An engine that keeps timing out at the 3 s ceiling is a candidate for
