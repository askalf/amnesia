...
Two honest caveats. Cloudflare's Bot Fight Mode injects an inline script of its own into every response, and this CSP refuses it — the page is unaffected, the console shows a violation, and the zone's JavaScript bot detection does not run on this site (the API is gated by Turnstile and a rate limit, not by bot scores, so nothing depends on it; turning Bot Fight Mode off for the zone removes the noise). And the VPN row on the live instance is enforced by **configuration, not by a network boundary**: SearXNG is told to use the proxy rather than living inside the VPN container's network namespace, so an engine that ignored the proxy environment would leave by the host IP. The hard shape is in the tree — [`infra/docker-compose.vpn.yml`](infra/docker-compose.vpn.yml) puts SearXNG inside its own gluetun's namespace (`network_mode: "service:gluetun"`), where the only route out is the tunnel and gluetun's firewall drops everything while the tunnel is down; verified on the host by trying to leave that namespace with the tunnel deliberately down (DNS fails, a direct-IP request times out) while the same request on the plain bridge leaves by the host IP. The live instance moves to it when a second VPN session is provisioned; until then the row above describes what runs.
