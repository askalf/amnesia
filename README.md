...
| Only two origins may run script | CSP `script-src 'self' https://challenges.cloudflare.com`, HSTS preload, `X-Frame-Options`, `Referrer-Policy: same-origin` ([`src/_headers`](src/_headers)) | `curl -sI https://amnesia.tax/ \| grep -i content-security-policy` |
